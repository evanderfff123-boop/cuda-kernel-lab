...
__global__ void transpose_padding_kernel(int m, float* a, float* c) {
    __shared__ float smemArray[THREADS_PER_BLOCK_Y][THREADS_PER_BLOCK_X + 1];
...
void launch_padding(int m, float* a, float* b, cudaStream_t stream) {
    const dim3 block(THREADS_PER_BLOCK_X, THREADS_PER_BLOCK_Y);
    const dim3 grid((m + block.x - 1) / block.x,
                    (m + block.y - 1) / block.y);
    transpose_padding_kernel<<<grid, block, 0, stream>>>(m, a, b);
}
